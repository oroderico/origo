...
class SeedToolVerifierTests(unittest.TestCase):
    ...
    def test_passphrase_keyboards_cover_printable_ascii(self):
        # Once per arrangement, not over both together: a character present
        # only in the QWERTY pages would leave a passphrase containing it
        # untypeable for a reader who had switched the keyboard to
        # alphabetical, which is the same failure as dropping it outright for
        # half the users. The firmware's own self-test asserts the same thing
        # against the compiled arrays; this reads the source, so the two agree
        # only if the definition really is what ships.
        source = (Path(__file__).parents[1] / "main/seedtool_app.c").read_text()
        arrays = {
            "seedtool_passphrase_layouts[PASSPHRASE_PAGES] = {": "QWERTY",
            "seedtool_passphrase_layouts_alpha[PASSPHRASE_PAGES] = {": "alphabetical",
        }
        for anchor, which in arrays.items():
            self.assertIn(anchor, source, f"the {which} passphrase array is not where this test looks for it")
            layouts = source.split(anchor, 1)[1].split("};", 1)[0]
            covered = set()
            for line in layouts.splitlines():
                ...
                covered.update(body.replace("\\n", "").replace("\\b", "").replace("\\t", "").replace("\\r", "")
                               .replace('\\"', '"').replace("\\\\", "\\"))
            self.assertEqual(covered, {chr(c) for c in range(0x20, 0x7F)}, f"the {which} pages do not cover ASCII")

    def test_word_keyboards_hold_every_letter(self):
        # Both arrangements of the letter keyboard, from the header where they
        # are defined. A layout with a letter missing makes some BIP39 words
        # impossible to type - the reader would reach for a key that is not
        # there and have no way to finish a restore.
        header = (Path(__file__).parents[1] / "main/seedtool_app.h").read_text()
        for name in ("SEEDTOOL_WORD_LAYOUT", "SEEDTOOL_WORD_LAYOUT_ALPHA"):
            match = re.search(rf'#define {name} "([^"]*)"', header)
            self.assertIsNotNone(match, f"{name} is not defined as a plain string literal")
            keys = match.group(1).replace("\\n", "").replace("\\b", "")
            self.assertEqual(sorted(keys), sorted(string.ascii_lowercase), f"{name} is not the 26 letters exactly once")
